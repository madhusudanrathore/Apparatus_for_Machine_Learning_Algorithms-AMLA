def Run():
	...
	if not os.path.exists('./GRU/model_info.txt'):
		...
		# load the dataset
		dataframe = read_csv('./GRU/airline.csv', usecols=[1], engine='python', skipfooter=3)
		...
		#The GRU network expects the input data (X) to be provided with a
		...
		model = Sequential()
		model.add(GRU(50, input_shape=(nn_look_back, 1)))
		...
		# save model info
		json.dump(results, open("./GRU/model_info.txt",'w'))

	else: # load data
		results=json.load(open("./GRU/model_info.txt"))
